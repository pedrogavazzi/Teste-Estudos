...
/**
 * Aba "Caderno": aulas de hoje, práticas para anotar durante a própria aula. Ao salvar uma
 * anotação, a aula sai de "Em andamento" e passa para "Aulas feitas" — mas continua editável.
 */
        ...
        if (semAulas) {
            ...
            LazyColumn(
                modifier = Modifier
                    .fillMaxSize()
                    .padding(padding)
                    // Tocar em qualquer área vazia da lista fecha o teclado e salva a anotação
                    // que estiver aberta no momento (comportamento de "tocar fora salva").
                    .pointerInput(Unit) {
                ...
                if (estado.emAndamento.isNotEmpty()) {
                    ...
                    items(estado.emAndamento, key = { "andamento_${it.aula.id}" }) { item ->
                        ItemCaderno(
                            ...
                            onSalvarAnotacao = { texto -> viewModel.salvarAnotacao(item.aula, texto) }
                        )
                    }
                }
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp)) {
            ...
            AnotacaoEditor(
                ...
                valorSalvo = item.aula.anotacoesCaderno,
                onSalvar = onSalvarAnotacao,
                rotulo = "Anotações da aula",
                minLinhas = 5,
                colapsavel = true,
                textoResumoVazio = "Toque para anotar sobre esta aula"
            )
        }
    }
}
